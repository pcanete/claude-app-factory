...
def validate_spec(spec: Any) -> list[str]:
    ...
    if not isinstance(roles, list) or not roles:
                ...
                if not isinstance(capabilities, list):
                    ...
                    for capability in capabilities:
                        if capability not in CAPABILITIES:
                            errors.append(
                                f"{path}.capabilities has an unknown capability: {capability}."
                            )
        ...
        if declared and not any("manage_users" in role["capabilities"] for role in declared):
            errors.append(
                "At least one role must declare the manage_users capability once capabilities are used."
            )
...
def compile_sql(spec: dict[str, Any]) -> str:
    lines = [
        "-- GENERATED BY CLAUDE APP FACTORY. CREATE NEW MIGRATIONS; DO NOT EDIT AFTER DEPLOYMENT.",
        "-- No transaction here: scripts/apply-migrations.mjs wraps this file and its",
        "-- ledger entry in one transaction so both commit or roll back together.",
        ...
        "",
        "-- RLS en todas las tablas, sin politicas: el dueno (la aplicacion) no queda sujeto a",
        "-- ellas, pero cualquier API de datos que el proveedor exponga sobre esta base queda",
        "-- cerrada por defecto en vez de depender de que nadie conozca una clave publica.",
        "ALTER TABLE app_role ENABLE ROW LEVEL SECURITY;",
        "ALTER TABLE app_user ENABLE ROW LEVEL SECURITY;",
        "ALTER TABLE app_audit_log ENABLE ROW LEVEL SECURITY;",
        "ALTER TABLE app_import_batch ENABLE ROW LEVEL SECURITY;",
        "ALTER TABLE app_attachment ENABLE ROW LEVEL SECURITY;",
        "",
        "-- search_path fijo: la resolucion de nombres no depende del rol que dispare el trigger.",
    ...
    for entity in spec["entities"]:
        ...
        lines.extend(
            [
                ...
                "  FOR EACH ROW EXECUTE FUNCTION app_set_updated_at();",
                f"ALTER TABLE {sql_identifier(entity['key'])} ENABLE ROW LEVEL SECURITY;",
                "",
            ]
        )
...
def compile_registry(spec: dict[str, Any]) -> str:
    ...
    return (
        "/* GENERATED BY CLAUDE APP FACTORY. DO NOT ADD CLIENT FEATURES HERE. */\n"
...
def compile_navigation(spec: dict[str, Any]) -> str:
    ...
    return (
        "/* GENERATED BY CLAUDE APP FACTORY. */\n"
...
def declared_capabilities(spec: dict[str, Any]) -> dict[str, list[str]] | None:
    """Capacidades administrativas por rol, o None si la AppSpec no las declara."""
...
def compile_permissions(spec: dict[str, Any]) -> str:
    ...
    # Sin declaracion explicita el runtime cae en la heuristica historica y el
    # BUILD_REPORT la reporta como puerta pendiente.
    literal = "null" if capabilities is None else typescript_literal(capabilities)
    return (
        "/* GENERATED BY CLAUDE APP FACTORY. ENFORCE THIS MATRIX SERVER-SIDE. */\n"
        f"export const generatedPermissions = {typescript_literal(matrix)} as const;\n\n"
        "/* Administrative capabilities per role. `null` means the AppSpec declared\n"
        "   none and the runtime falls back to the legacy all-permissions heuristic. */\n"
...
def compile_report(spec: dict[str, Any]) -> str:
    ...
    if capabilities is None:
        capability_note = (
            "- Administrative access (user management, audit, rules) is inferred from holding "
            "every permission on every entity; no role declares it."
        )
        capability_gate = (
            "- Declare `capabilities` on the roles that may manage users, read the audit log, and "
            "review rules. Until then any role with full entity permissions inherits administration."
        )
    else:
        administrators = sorted(
            key for key, granted in capabilities.items() if "manage_users" in granted
        )
        capability_note = (
            "- Administrative access is declared per role in the AppSpec; user management is "
            f"limited to: {', '.join(administrators)}."
        )
        capability_gate = (
            "- Confirm with the client which roles hold `manage_users`, `view_audit`, and "
            "`view_rules`; changing them requires a new AppSpec version."
        )
    lines = [
...
This directory belongs to the client application and is never written by the factory.
Add client-specific calculations, integrations, workflows, reports, AI tools, and
bespoke UI here.

Do not edit `src/generated/` (compiled from `app-spec.json`) or `src/platform/`
(shipped and updated by the factory). To change generated structure, change the
AppSpec and recompile. To change platform behavior, import it and wrap it here.

`src/platform/ai/` contains the neutral read-only application assistant. Extend its
tool registry from a feature adapter; never bypass application permissions with
direct SQL.

`src/platform/auth/adapter.ts` uses Clerk only as the production identity boundary.
Active status, AppSpec roles, permissions, and invitation provisioning stay in
PostgreSQL. Client-specific invitation policy belongs here, not there.
"""


PLATFORM_GUIDE = """# Platform code

This directory ships with Claude App Factory: identity adapter, application settings,
user administration, and the read-only assistant. It is neither compiled from the
AppSpec nor owned by the client, and a future factory version may replace it.

Do not edit these files. Client behavior goes in `src/features/`; generated structure
comes from `app-spec.json`.
"""


CUSTOM_DB_GUIDE = """# Custom database changes

Put client-specific migrations here, in a numbering sequence starting at 500 so they
always apply after generated and platform migrations.

Never edit an applied migration in `database/generated/` or `database/platform/`; add
a new reviewed migration with an explicit dependency. Migration files must not open
their own transaction: `scripts/apply-migrations.mjs` wraps each file and its ledger
entry in one transaction so they commit or roll back together.
"""


PLATFORM_DB_GUIDE = """# Platform migrations

These migrations ship with Claude App Factory and support identity, user management,
application settings, and the assistant. They are not client migrations: do not edit
them, and keep client changes in `database/custom/`.
"""
...
def scaffold(spec: dict[str, Any], output: Path) -> list[Path]:
    ...
    files: dict[str, str] = {
        ...
        "src/platform/OWNERSHIP.md": PLATFORM_GUIDE,
        "database/custom/EXTENSIONS.md": CUSTOM_DB_GUIDE,
        "database/platform/OWNERSHIP.md": PLATFORM_DB_GUIDE,
